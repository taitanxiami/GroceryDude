...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"LocationAtShop Cell" forIndexPath:indexPath];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        //删除数据源
        LocationAtShop *removeLocationAtShop = [self.frc objectAtIndexPath:indexPath];
        [self.frc.managedObjectContext deleteObject:removeLocationAtShop];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    //传值
    LocationAtShopVC *shop = segue.destinationViewController;
    
    if ([segue.identifier isEqualToString:@"Add Object Segue"]) {
        
        CoreDataHelper *cdh = [(AppDelegate *)[UIApplication sharedApplication].delegate  cdh];
        LocationAtShop *unit = [NSEntityDescription insertNewObjectForEntityForName:@"LocationAtShop" inManagedObjectContext:cdh.context];
